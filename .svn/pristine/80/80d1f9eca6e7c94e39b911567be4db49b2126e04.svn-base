...
module CsvImportDb
  ...
  def CsvImportDb.clean_after_import(tname, path = "/tmp/", tariff_id = '')
  ...
  def CsvImportDb.save_file(id, file, path = "/tmp/")
    ...
    cmd_clid_to_source_cleaner = "/usr/local/m2/m2_clean_src_in_csv #{full_file_path}"
    MorLog.my_debug("CSV save_file #{tname}: running command: #{cmd_clid_to_source_cleaner}", 1)
    cmd_clid_to_source_cleaner_response = system(cmd_clid_to_source_cleaner)
    MorLog.my_debug("CSV save_file #{tname}: command output: #{cmd_clid_to_source_cleaner_response}", 1)
  ...
  def CsvImportDb.load_csv_into_db(tname, sep, dec, fl, path = "/tmp/", options = {})
    ...
    replace_tmp_table_headers(tname, cols_size)
    ...
    if options[:xml]
      ...
      load += " FIELDS TERMINATED BY '#{(sep == "'") ? '\\' + sep : sep}' "
      load += " OPTIONALLY  ENCLOSED BY '\"' "
      load += " lines terminated by '\n' "
      load += " IGNORE #{options[:lines_to_skip].to_i} LINES " if options[:lines_to_skip]
      load += strip_columns
      load += ";"
    end
  ...
  def self.tariff_job_temp_import(structure, file_to_import_path)
    ActiveRecord::Base.connection.execute("DROP TABLE IF EXISTS #{structure[:name]};")

    sql = "CREATE TABLE #{structure[:name]} ("
    structure[:columns].each do |column|
      sql << "#{column[:name]} #{column[:type]} #{column[:additional]}, "
    end
    sql << structure[:keys].join(', ')
    sql << ') ENGINE=InnoDB DEFAULT CHARSET=utf8;'
    ActiveRecord::Base.connection.execute(sql)

    load = "LOAD DATA LOCAL INFILE '#{file_to_import_path}' IGNORE INTO TABLE #{structure[:name]} CHARACTER SET utf8"
    load << " FIELDS TERMINATED BY ',' "
    load << " OPTIONALLY ENCLOSED BY '\"' "
    load << " LINES TERMINATED BY '\n' "
    load << ' IGNORE 1 LINES '
    load << " (#{structure[:import_columns].join(',')}) ;"

    ActiveRecord::Base.connection.active?
    ActiveRecord::Base.connection.execute(load)
  end

  def self.prefix_temp_import(structure, file_to_import_path)
    ActiveRecord::Base.connection.execute("DROP TABLE IF EXISTS #{structure[:name]};")
    ...
    load = "LOAD DATA LOCAL INFILE '#{file_to_import_path}' IGNORE INTO TABLE #{structure[:name]} CHARACTER SET utf8"
    load << " FIELDS TERMINATED BY '#{Confline.get_value('CSV_Separator')}' "
    load << " OPTIONALLY ENCLOSED BY '\"' "
    load << " LINES TERMINATED BY '\n' "
    load << ' IGNORE 1 LINES '
    load << " (#{structure[:import_columns].join(',')}) ;"
