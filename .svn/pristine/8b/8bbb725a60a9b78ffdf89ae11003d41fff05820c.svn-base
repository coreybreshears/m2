window.addEventListener('load', function() {
	...
	for (var i = 0; i < sliderRadios.length; i++) {
		var sliderRadio = sliderRadios[i];
		...
		// bugs/issues
		// 1. strange click behaviour when used in initialization. Causes other radio buttons not saving their value
		//   potencial fix: create empty event and call it instead of click
		// 2. same variable for nested loop causes infinite loop
		//  fix: change variable name to j
		// 3. dosent work properly if radios are hidden
		//  fix: check handler function to investigate issue
		// const radios = sliderRadio.querySelectorAll("input[type='radio']");
		// for(var i = 0; i < radios.length; i++) {if(radios[i].checked) radios[i].click() }
	}
