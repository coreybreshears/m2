...
SEND_EMAIL="/usr/local/m2/sendEmail"
...
if [ "$MODE" == "0" ]; then
	...
	E_MAIL_RECIPIENTS="info@ocean-tel.uk m.mardosas@gmail.com martynas.margis@gmail.com tankiaitaskuota@gmail.com" #separate each address with a space
else
...
change_email_in_environment_rb()
{
    ...
    sed 's/guicrashes@ocean-tel.uk\|support@ocean-tel.uk/ocean.mindaugas.crm@gmail.com/g'  /home/mor/config/environment.rb > /home/mor/config/environment.rb2
...
send_report_by_email()	{
	if [ -f "$SEND_EMAIL" ]; then
		...
		if [ "$STATUS" == "OK" ]; then
			$SEND_EMAIL -f mor_tests@ocean-tel.uk -t $E_MAIL_RECIPIENTS -u "[$STATUS][MOR TESTS C1 $MOR_VERSION_YOU_ARE_TESTING] $CURRENT_REVISION $mor_time" -m "REVISION: $CURRENT_REVISION  LAST AUTHOR: $LAST_AUTHOR  STATUS: $STATUS     `cat $report`"  -o reply-to=mor_tests@ocean-tel.uk tls=auto -s smtp.gmail.com -xu kolmitest -xp kolmitest99 > /tmp/mor_temp

		elif [ "$STATUS" == "FAILED" ]; then
			$SEND_EMAIL -f mor_tests@ocean-tel.uk -t $E_MAIL_RECIPIENTS -u "[$STATUS][MOR TESTS C1 $MOR_VERSION_YOU_ARE_TESTING] $CURRENT_REVISION $mor_time" -m "REVISION: $CURRENT_REVISION  LAST AUTHOR: $LAST_AUTHOR  STATUS: $STATUS `cat $report`" -a $MOR_CRASH_LOG -o reply-to=mor_tests@ocean-tel.uk tls=auto -s smtp.gmail.com -xu kolmitest -xp kolmitest99 > /tmp/mor_temp
		fi
...
test_if_all_tests_were_converted_successfully(){
	...
	if [ $RB_FILES -ne $CASE_FILES ];
		then
			...
			$SEND_EMAIL -f mor_tests@ocean-tel.uk -t $E_MAIL_RECIPIENTS -u "[FAILED][MOR TESTS $MOR_VERSION_YOU_ARE_TESTING] $mor_time" -m "REVISION: $CURRENT_REVISION  LAST AUTHOR: $LAST_AUTHOR  STATUS: FAILED TO CONVERT THE TESTS" -o reply-to=mor_tests@ocean-tel.uk tls=auto -s smtp.gmail.com -xu kolmitest -xp kolmitest99 > /tmp/mor_temp
...
if [ -z "$1" ];
	then
        ...
        echo -e "\t-rm \tRemove lock\n";
				...
				svn co http://svn.ocean-tel.uk/mor/install_script/trunk/ /usr/src/mor
				chmod +x /usr/bin/mor
    ...
    if [ "$SECOND_GLOBAL_PARAM" == "8" ]; then
        ...
        if [ "$MOR_VERSION_YOU_ARE_TESTING" == "trunk" ]; then
            svn co http://svn.ocean-tel.uk/mor/gui/trunk/ /home/mor
        else
            svn co http://svn.ocean-tel.uk/mor/gui/branches/$MOR_VERSION_YOU_ARE_TESTING /home/mor
        fi
    fi
