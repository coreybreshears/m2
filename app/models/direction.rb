...
class Direction < ActiveRecord::Base
  attr_accessible :id, :name, :code, :iso31661code

  has_many :destinations, -> { order(:prefix) }, foreign_key: 'direction_code', primary_key: 'code'
  has_one  :blocked_country
  ...
  def Direction::get_direction_by_country(country)
    ...
    dir ? dir.id.to_i : Confline.get_value('Default_Country_ID')
  end
  ...
  def Direction.directions_order_by(options)
