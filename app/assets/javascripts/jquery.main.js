...
function initCustomHover() {
	...
	jQuery('#nav>ul>li>ul>li>ul>li').touchHover();
}
...
function initCloseBlock() {
    ...
    jQuery('.login-success-box').each(function() {
        var hold = jQuery(this);
        var closeLink = hold.find('.close');
        closeLink.on('click', function(e) {
            e.preventDefault();
...
function initDateOnlypicker() {
	...
	function parseDateTime(dateString) {
		var time_format;
		if(dateString.match(/.+\d+:\d+:\d+/) == dateString)
			time_format = 'HH:mm:ss';
		else time_format = 'HH:mm'
		date = jQuery.datepicker.parseDateTime(date_format, time_format, dateString);
		return date;
	}
	function getStartDate() {
		dateString = jQuery('#from').attr('value')
        try {
	...
	function getEndDate() {
        dateString = jQuery('#till').attr('value')
        try {
        	return parseDateTime(dateString);
        } catch(err) {
        	return ''
	...
	jQuery('.calendar-holder').each(function(){
		var set = jQuery(this);
		var checkin = set.find('.checkin3');
		var checkout = set.find('.checkout3');
		var issue = set.find('.issue3');
		var checkoutWithSeconds = set.find('.checkout-seconds');
		var startDate = false;
		var endDate = false;
		var value;
		var value2;
		...
		checkinCalendar = [checkin.parent().find('.date-link')];
		...
		checkoutCalendar = [checkout.parent().find('.date-link')];
		jQuery.each(checkoutCalendar, function(){
			jQuery(this).click(function() {
				checkout.trigger('select');
			});
		});
		issueCalendar = [issue.parent().find('.date-link')];
		jQuery.each(issueCalendar, function(){
			jQuery(this).click(function() {
				issue.trigger('select');
			});
		});
		setDateTimePickerDate(checkin, getStartDate());
		end_date = getEndDate();
		if(end_date != ''){
			setDateTimePickerDate(checkout, end_date);
		}
	})
}
...
function initTimepicker() {
    //wrapper functions to make timepicker initialization easier
    function dateBeginningOfDay(date) {
        date.setHours(0,0,0);
        return date;
    }
    ...
    function getStartDate() {
        dateString = jQuery('#from2').attr('value') + ' ' + jQuery('#time12').attr('value');
        return parseDateTime(dateString);
    }
    function getEndDate() {
        var till_value = jQuery('#till2').attr('value');
        var time2_value = jQuery('#time22').attr('value');
        dateString = till_value + ' ' + time2_value;
        if(till_value != null && time2_value != null)
            return parseDateTime(dateString);
        else return '';
    }
    function setDateTimePickerDate(element, date) {
    ...
    jQuery('.time-holder').each(function(){
        var set = jQuery(this);
        var checkin = set.find('.checkin2');
        var checkout = set.find('.checkout2');
        var startDate = false;
        var endDate = false;
        var value;
        var value2;
        var checkin_noSeconds = (checkin.hasClass('_noSeconds') ? 'HH:mm' : 'HH:mm:ss');
        checkin_noSeconds = (checkin.hasClass('_noMinutes') ? 'HH' : checkin_noSeconds);
        var checkout_noSeconds = (checkout.hasClass('_noSeconds') ? 'HH:mm' : 'HH:mm:ss');
        function calcTop(input) {
            var row = jQuery(input).parent();
            value = row.outerHeight() + row.offset().top;
            value2 = row.offset().left;
        ...
        checkin.timepicker({
            ...
            onClose: function( selectedDate ) {
                checkout.timepicker( "option", "minDate", selectedDate );
            },
            beforeShowDay: function(date){
                calcTop(checkin);
                setTimeout(function () {
        ...
        checkout.timepicker({
            ...
            onClose: function( selectedDate ) {
                checkin.timepicker( "option", "maxDate", selectedDate );
            },
            beforeShowDay: function(date){
                calcTop(checkout);
                setTimeout(function () {
        ...
        checkinCalendar = [checkin.parent().parent().find('.clock-link2')];
        jQuery.each(checkinCalendar, function(){
            jQuery(this).click(function() {
        ...
        checkoutCalendar = [checkout.parent().parent().find('.clock-link2')];
        jQuery.each(checkoutCalendar, function(){
            jQuery(this).click(function() {
                checkout.trigger('select');
            });
...
function initDatepicker() {
    ...
    function getStartDateMe() {
        dateString = jQuery('#fromme').attr('value') + ' ' + jQuery('#timeme').attr('value');
        try {
            return parseDateTime(dateString);
        } catch(err) {
            return ''
    ...
    function getEndDateMe() {
        dateString = jQuery('#fromme2').attr('value') + ' ' + jQuery('#timeme2').attr('value');
        try {
            return parseDateTime(dateString);
        } catch(err) {
            return ''
	...
	jQuery('.calendar-holder').each(function(){
		...
		var checkoutWithSeconds = set.find('.checkout-seconds');
		var hide_secs = set.find('.hide_secs');
        var checkinme = set.find('.checkinme');
        var checkoutme = set.find('.checkoutme');
		var startDate = false;
		var endDate = false;
		var value;
		var value2;
		...
		checkin.datetimepicker({
			...
			onClose: function( selectedDate ) {
					var end_Time = jQuery('#time2').attr('value');
			    checkout.datetimepicker( "option", "minDate", selectedDate );
			    jQuery('#time2').attr('value', end_Time);
			},
			beforeShowDay: function(date){
				calcTop(checkin);
				setTimeout(function () {
		...
		checkout.datetimepicker({
			dateFormat: date_format,
			minDateTime: jQuery('.no-past').length != 0 ? new Date(new Date().setHours(0,0,0)) : new Date(2005, 1, 1),
			altField: "#time2",
			timeFormat: "HH:mm",
			stepMinute: 1,
		...
		checkoutWithSeconds.datetimepicker({
			dateFormat: date_format,
			altField: "#time2",
			timeFormat: "HH:mm:ss",
			stepMinute: 1,
			showSecond: hide_secs ? false : true,
			secondMax: hide_secs ? 0 : 59,
        ...
        checkinmeCalendar = [checkinme.parent().find('.calendar-link'), checkinme.parent().parent().find('.clock-link')];
        jQuery.each(checkinmeCalendar, function(){
            jQuery(this).click(function() {

                checkinme.trigger('select');
            });
        });
        checkoutmeCalendar = [checkoutme.parent().find('.calendar-link'), checkoutme.parent().parent().find('.clock-link')];
        jQuery.each(checkoutmeCalendar, function(){
            jQuery(this).click(function() {
                checkoutme.trigger('select');
            });
        });
		setDateTimePickerDate(checkin, getStartDate());
		end_date = getEndDate();
        ...
        setDateTimePickerDate(checkinme, getStartDateMe());
        end_date = getEndDateMe();
        if(end_date != ''){
            setDateTimePickerDate(checkoutme, end_date);
        }
	})
}
...
;(function($){
	...
	if(isTouchDevice || isWinPhoneDevice) {
		...
		toggleOn = function(e) {
            ...
            element_siblings.removeClass(options.hoverClass);
			element.addClass(options.hoverClass);
		...
		toggleOff = function(e) {
			var options = e.data, element = $(this);
            timeouts.push(setTimeout(function(){element.removeClass(options.hoverClass);}, 350));
